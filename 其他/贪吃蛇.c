//贪吃蛇游戏要求：
//（1）产生一个固定大小没有边界的游戏区域，蛇从区域的中心开始，由玩家通过键盘控制蛇的运动方向，用蛇头去吃随机分布在游戏区域内的食物；
//     蛇的运动限制在游戏区域内，游戏区域没有边界，所以蛇在区域内作循环运动；
//（2）蛇的运动方向为直线运动，只走横和竖的方向，不走斜线；
//（3）蛇的运动速度由游戏的难度来控制，难度越高，速度越快，游戏难度分为9个等级；
//（4）蛇身体的长度从1开始每吃掉一份食物就增加一个长度；
//（5）食物的出现安照随机分布的原则，蛇吃掉一份后随即在游戏区域内放一份新的食物；
//（6）每吃掉一份食物得分为10，游戏结束后统计全部的得分；
//（7）游戏结束的条件为：在控制蛇的过程中蛇头碰到蛇的身体的任何部位
